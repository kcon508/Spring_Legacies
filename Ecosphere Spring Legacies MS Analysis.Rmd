...
fact.cols<-c("Plot","Trt","TrtName","NewTrt","Comb.TRT")
...
dat<-dat%>%
  ...
  select(new.Date,deluge_period,Plot,NewTrt,Comb.TRT,VWC,Unique_ID,Notes)%>%
...
# ggsave(vwc.plot,filename="MS_SGS_2021_VWC_lines.pdf",bg="transparent",width=11.5,height=5)
...
## Export plot
# ggsave(vwc.avs.del.periods,filename="MS_SGS_2021_VWC_periodbars.pdf",bg="transparent",height=5,width=5)
...
# Convert categorical columns
fact.cols<-c("Plot","Trt","TrtName","NewTrt")
gcc.dat<-gcc.dat%>%
  mutate_at(fact.cols,factor)
...
dat<-gcc.dat%>%
  ...
  mutate(deluge_period=case_when(
    ...
    new.Date %within% interval ("2021-07-16","2021-08-06") ~ "exp.del.resp",
    new.Date %within% interval ("2021-08-07","2021-09-30") ~ "post.del"))%>%
  select(new.Date,deluge_period,Plot,NewTrt,GCC,Unique_ID,Notes)%>%
  mutate(deluge_period=factor(deluge_period,levels=c("pre.del","nat.del.resp","exp.del.resp","post.del")))%>%
  mutate(Comb.TRT=case_when(
...
# ggsave(gcc.plot,filename="MS_SGS_2021_GCC_lines.pdf",bg="transparent",width=11.5,height=5)
...
## Export plot
# ggsave(gcc.avs.del.periods,filename="MS_SGS_2021_GCC_periodbars.pdf",bg="transparent",width=5,height=5)
```
...
resp<-resp.dat%>%
  mutate(deluge_period=case_when(
    ...
    new.Date %within% interval ("2021-07-16","2021-08-06") ~ "exp.del.resp",
    new.Date %within% interval ("2021-08-07","2021-09-30") ~ "post.del"))%>%
  select(new.Date,deluge_period,Plot,NewTrt,Target_CO2,Time,Soil_Temp,Efflux,Unique_ID,Notes)%>%
  mutate(deluge_period=factor(deluge_period,levels=c("pre.del","nat.del.resp","exp.del.resp","post.del")))%>%
  mutate(NewTrt=factor(NewTrt,levels=c("amb","amb w/ deluge","dry w/ deluge")))%>%
...
# ggsave(efflux.plot,filename="MS_SGS_2021_Efflux_lines.pdf",bg="transparent",width=11.5,height=5)
...
## Export plot
# ggsave(efflux.avs.del.periods,filename="MS_SGS_2021_Efflux_periodbars.pdf",bg="transparent",width=4,height=5)
```
...
# name="Plots/MS_Finals/MS_SGS_2021_Regrowth_ANPP_3trts.pdf"
# letters<-c("a","b","a")

## Turn on for Sept Plot
gr_dat<-subset(anpp.avs,anpp.avs$Clip_Type=="September")
name="MS_SGS_2021_September_ANPP.pdf"
letters<-c("ab","a","b")
...
# Read in data
prs.dat<-read.csv('SGS_SLs_PRS_droppedlow.csv')
str(prs.dat)
...
NO3.plot<-ggplot(data=gr_dat,aes(x=TRT,y=av,fill=TRT,color=TRT))+
  ...
  ylab(expression(bold(atop("PRS Probe Uptake of NO"[3],"(µg 10cm"^-1*" 2 month burial"^-1*")"))))+
  xlab("")+
...
## Export plot
# ggsave(sentek.avs.del.periods,filename="MS_SGS_2021_Sentek_periodbars.pdf",bg="transparent",width=5,height=5)
...
# ggsave(sentek_avs,filename="MS_SGS_2021_sentek_heatmap.pdf",width=10,height=3.5)
```
...
# ggsave(monthly.precip.bars,filename="MS_2021_Fig1_Precip_bars.pdf",bg="transparent",width=9,height=5)
...
# ggsave(hist,filename="MS_2021_Fig1_Deluge_hist.pdf",bg="transparent",width=5.5,height=5)
